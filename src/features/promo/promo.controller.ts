...
export const validatePromoCode = async (req: Request, res: Response) => {
  try {
    ...
    if (!safeBody.success) {
      logger(
        "Invalid parameters for promo code validation:",
        safeBody.error.issues[0]?.message || "Invalid request body",
      );
      return res
        .status(400)
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to validate promo code" });
    }
    ...
    if (!result.valid) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Promo code is not valid" });
    }
