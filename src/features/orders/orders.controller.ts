...
export const getAllOrders = async (
  ...
  try {
    ...
    let query = supabase
      .from("orders")
      .select(
        "*, address:addresses(full_name, address_line, city, country), shipping_method:shipping_methods(name, carrier, estimated_days), items:order_items(*)",
        { count: "exact" },
      )
      .eq("user_id", userId)
    ...
    if (error) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Cannot Get Orders" });
    }
...
export const getOrderById = async (
  ...
  try {
    ...
    const { data: order, error } = await supabase
      .from("orders")
      .select(
        "*, address:addresses(full_name, address_line, city, state, country, phone, zip_code), shipping_method:shipping_methods(name, carrier, estimated_days), promo_code:promo_codes(code, type, value), items:order_items(*)",
      )
      .eq("id", orderId)
      .eq("user_id", userId)
      .single();
    ...
    if (error) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Cannot Get Order", error });
    }
...
export const createOrder = async (
  ...
  try {
    ...
    const { addressId, shippingMethodId, promoCode, paymentMethod, notes } =
      safeBody.data;
    ...
    const rpcArgs = {
      ...
      ...(promoCode !== null && promoCode !== undefined
    ...
    if (paymentMethod === "credit_card") {
      ...
      const { error: updateErrorWithStripe } = await supabase
        .from("orders")
        .update({
    ...
    const { error: updateErrorWithNonStripe } = await supabase
      .from("orders")
      .update({
        payment_method: paymentMethod,
        notes: notes ?? null,
...
export const cancelOrder = async (
  ...
  try {
    ...
    if (error) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Cannot Cancel Order" });
    }
