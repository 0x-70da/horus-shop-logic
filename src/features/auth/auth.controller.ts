  ...
  try {
    ...
    if (!safeBody.success) {
      ...
      return res.status(400).json({
        success: false,
        message:
          safeBody.error.issues[0]?.message || "Invalid registration data",
      });
    }
    ...
    if (selectError) {
      logger(
        "Error checking existing user in register controller:",
        selectError,
      );
...
export const login = async (
  ...
  try {
    ...
    if (updateError) {
      logger(
        "Error updating refresh token in database for user ID:",
        user.id,
        "Error:",
        updateError,
      );
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (updateError) {
      logger(
        "Error updating reset token in database for user ID:",
        user.id,
        "Error:",
        updateError,
      );
...
export const verifyCodeOrResetToken = async (req: Request, res: Response) => {
  try {
    ...
    if (dbError) {
      logger(
        "Error fetching user in verifyCodeOrResetToken controller:",
        dbError,
      );
      return res
        .status(500)
    ...
    const isExpired =
      user.reset_expires_at && new Date(user.reset_expires_at) < new Date();
    if (isExpired) {
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (updateError) {
      logger(
        "Error updating password in database for user ID:",
        user.id,
        "Error:",
        updateError,
      );
...
export const refresh = async (req: Request, res: Response) => {
  try {
    ...
    const { error: updateError } = await supabase
      .from("users")
      .update({ refresh_token: hashedNewRefreshToken })
      .eq("id", decoded.id);
...
export const me = async (req: Request, res: Response) => {
  try {
    ...
    const { data: user, error: fetchError } = await supabase
      .from("users")
      .select("id, email, avatar, first_name, last_name, role")
      .eq("id", userId)
      .maybeSingle();
